...
export default class PeopleProvider {
  ...
  public async generateContactsCsv(
    ...
    // Clone contacts
    const contacts = JSON.parse(JSON.stringify(unsortedContacts)) as people_v1.Schema$Person[];
    ...
    function formatGroupMemberships(
      contactGroups: people_v1.Schema$ContactGroup[],
    ...
    function formatEvents(
      ...
      for (const [i, event] of (events ?? []).entries()) {
        ...
        result[`Event ${i + 1} - Value`] = formatDate(event.date);
      }
      return result;
    }
    ...
    const records = contacts.map((contact) => ({
      ...
      Photo: contact.photos?.[0].url?.includes("/contacts/")
        ? contact.photos[0].url.replace(/=s100$/, "")
        : undefined,
      "Group Membership": formatGroupMemberships(contactGroups, contact.memberships),
      ...formatEmailAddresses(contact.emailAddresses),
      ...formatPhoneNumbers(contact.phoneNumbers),
      ...formatAddresses(contact.addresses),
      ...formatOrganizations(contact.organizations),
      ...formatRelations(contact.relations),
      ...formatUrls(contact.urls),
      ...formatEvents(contact.events),
    }));
